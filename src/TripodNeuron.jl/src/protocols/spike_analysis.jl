...
get_spikes(voltage::Array{Float32,1})::Array{Bool,1} =
    [v == AP_membrane for v in voltage[:]]
...
##
get_spike_times(voltage::Array{Float32,2}) =
    findall(x -> x == AP_membrane, get_spikes(voltage))
...
get_spike_rate(v::Array{Float32,2})::Float32 =
    float(sum(v[1, :] .== AP_membrane) / length(v[1, :]) * 10000)
get_spike_rate(v::Array{Float32,1}) = sum(v .== AP_membrane) / length(v) * 10000
get_spike_rate(v::Array{Float32,3}) =
    [sum(v[i, 1, :] .== AP_membrane) / length(v[i, 1, :]) * 10000 for i = 1:size(v, 1)]
