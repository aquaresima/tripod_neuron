...
function get_PSP_peak(voltage::Array{Float32,2}; spiketime=-1, rest=-70.6, IPSP=false, compartment=1)
...
function make_rates(simtime::Int,β=0, dt=0.1; rate::Real=1000., seed=0, kwargs...)
	"""
...
function make_spikes(simtime, β=0, ; rate=1000., ie_ratio=ones(3), kwargs...)
	...
	for x in 1:simtime
		for y in 1:round(Int,1/dt)
			...
			for i in 2:3
				...
				inh_spikes[i,z] = rand(Poisson(ie_ratio[i]*r_exc[i,x]*dt/1000))
			end
		end
	end
...
function make_balance(simtime; rate=3000, ie_ratio=[0., 1., 1,], model= nothing, kwargs...)
...
#
get_spikes(voltage::Array{T,1})  where T<:Real = [v == postspike.AP_membrane for v in voltage[:]]
...
get_spike_rate(voltage:: Vector{T}) where T<:Real = length(get_spike_times(voltage))/length(voltage) * 1000/dt
