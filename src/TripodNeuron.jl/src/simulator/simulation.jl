...
function run_tripod(
    ...
    postspike = PostSpike(),
    synapses = [1.0, 1.0, 1, 1.0, 1.0, 1.0],
    ...
    for tt = 1:total_steps
        ...
        if last_event < 0
            ...
            if !do_spikes
                ...
                v_soma +=
                    dt * ΔvSoma_nospike(v_soma, w_soma, sum(c_soma), (@view syn_soma[:, 2]))
            else
                ...
                if v_soma >= AdEx.θ
                    ...
                    v_dend1 +=
                        dt * ΔvDend(v_dend1, -c_soma[1], (@view syn_dend1[:, 2]), pm1)
                    v_dend2 +=
                        dt * ΔvDend(v_dend2, -c_soma[2], (@view syn_dend2[:, 2]), pm2)
        ...
        elseif last_event > idle_dt
            ...
            v_dend1 +=
                dt *
                ΔvDend(v_dend1, -c_soma[1], (@view syn_dend1[:, 2]), pm1) *
                postspike.BAP_gax
            v_dend2 +=
                dt *
                ΔvDend(v_dend2, -c_soma[2], (@view syn_dend2[:, 2]), pm2) *
                postspike.BAP_gax
