...
for (n, (k, v)) in enumerate(NMDA)
    p = heatmap(TN.νs, βs, v.output_rates[:, :, 1]', clims = (0, 15), c = :roma)
    push!(plotsNMDA, p)
...
p = plot(plotsNMDA..., xscale = :log, clims = (0, 15))
q = plot(plotsAMPA..., xscale = :log, clims = (0, 15))
...
savefig(zz, joinpath(path, "Fig6D.pdf"))
