...
@warn "This is the high resolution version of the script, it will only run for diagonal models"

@assert TN.name == "_highres"
...
for nmda in [true, false]
    ...
    for f in eachindex(νs)
        ...
        Threads.@threads for d1 in eachindex(TN.ds)
            ...
            if d1 == 1 || d2 == 1
                model =
                    (ds = (TN.ds[1], TN.ds[1]), nmda = nmda, soma_only = sum(TN.ds[1]) == 0)
            end
            # rate balance
            model =
                (ds = (TN.ds[d1], TN.ds[d2]), nmda = nmda, soma_only = sum(TN.ds[d1]) == 0)
            ...
            v_nospikes =
                TN.run_tripod(inputs, simtime; model..., do_spikes = false, cond...)
            istdp_mean_v[:, d1, d2, f] = mean(v_nospikes[:, 200_000:end], dims = 2)[:, 1]
    ...
    filep = datadir("up_down", "activity", "full_activity_$(syn)$(TN.name).jld2")
    safesave(filep, data)
end
